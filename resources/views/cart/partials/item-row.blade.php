        ...
        <div class="flex justify-between items-start">
            <div>
                <h3 class="text-lg font-semibold text-gray-900">
                    <a href="#" class="hover:text-indigo-600 transition-colors">{{ $item->product?->name ?? 'Unknown Product' }}</a>
                </h3>
                <p class="mt-1 text-sm font-medium text-gray-500">{{ $item->product?->category?->name ?? 'Uncategorized' }}</p>
            </div>
            <p id="item-price-{{ $item->product?->id }}" class="text-lg font-bold text-gray-900">
                ${{ number_format($item->product?->price ?? 0, 2) }}</p>
        </div>

        <div class="mt-6 flex items-center justify-between">
            <form x-data="cartItemHandler({{ $item->quantity }}, {{ $item->product->id }})" x-ref="updateForm" action="{{ route('cart.update', $item->product->id) }}"
                method="POST" class="flex items-center" @submit.prevent="updateCart()">
                @csrf
                @method('PATCH')
                <div
                    class="flex items-center border border-gray-200 rounded-full bg-white overflow-hidden shadow-sm h-9 relative">

                    <div x-show="loading" class="absolute inset-0 bg-white items-center justify-center z-10 hidden">
                    ...
                    <button type="button"
                        ...
                        :class="{
                            'opacity-50 cursor-not-allowed': qty <= 1 || loading,
                            'hover:text-gray-900': qty > 1 && !
                                loading
                        }"
                        :disabled="qty <= 1 || loading" @click="if(qty > 1 && !loading) { qty--; updateCart(); }">
                    ...
                    <input type="number" name="quantity" x-model.number="qty" min="1"
                        ...
                        :disabled="loading" @change="if(qty >= 1 && !loading) updateCart();">
                    ...
                    <button type="button"
                        ...
                        :disabled="loading" @click="if(!loading) { qty++; updateCart(); }">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            stroke-width="2.5">
            ...
            <div x-data="{ showConfirm: false }">
                <form x-ref="removeForm" action="{{ route('cart.remove', $item->product->id) }}" method="POST"
                    @submit.prevent="showConfirm = true">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        class="text-sm font-medium text-red-600 hover:text-red-800 transition-colors flex items-center group">
                        <svg class="w-4 h-4 mr-1 text-red-500 group-hover:text-red-700" fill="none"

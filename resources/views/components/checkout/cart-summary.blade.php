@props(['cartItems', 'subtotal' => 0, 'shippingFee' => 0, 'total' => 0])
...
<div class="bg-white border border-[#e4e4e7] shadow-sm rounded-2xl p-6 lg:sticky lg:top-8">
    ...
    <div class="space-y-3 text-sm text-[#71717a]">
        <div class="flex justify-between">
            ...
            <span class="font-medium text-[#27272a]"
                x-text="typeof formatPrice === 'function' && typeof subtotal !== 'undefined' ? formatPrice(subtotal) : '${{ number_format($subtotal, 2) }}'">${{ number_format($subtotal, 2) }}</span>
        </div>
        <div class="flex justify-between">
            <span>Shipping</span>
            <span class="font-medium text-[#27272a]"
                x-text="typeof shippingFee !== 'undefined' ? (parseFloat(shippingFee) === 0 ? 'Free' : formatPrice(shippingFee)) : '{{ $shippingFee == 0 ? 'Free' : '$' . number_format($shippingFee, 2) }}'">
        ...
        <div class="flex justify-between items-center">
            <span class="flex items-center gap-1">
                Estimated taxes
                <svg class="w-3.5 h-3.5 text-[#a1a1aa]" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    title="Taxes calculated at checkout">
    ...
    <div class="flex justify-between items-center mb-2">
        ...
        <span class="font-bold text-2xl text-[#27272a]"
            x-text="typeof formatPrice === 'function' && typeof total !== 'undefined' ? formatPrice(total) : '${{ number_format($total, 2) }}'">${{ number_format($total, 2) }}</span>
    </div>
</div>
